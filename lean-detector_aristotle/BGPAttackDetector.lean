...
def ofString? (s : String) : Option Path :=
  ...
  | [] => acc
  | h :: t => aux (Path.cons (h.toNat?.getD 0) acc) t
  aux Path.nil fields
...
/-
Commented out: axiom not allowed by verification tooling.
axiom mask_le (p1 p2 : Fin 33) (h : p1.val <= p2.val)
  : (Subnet.mask p2) &&& (Subnet.mask p1) = Subnet.mask p1
-/
theorem mask_le (p1 p2 : Fin 33) (h : p1.val <= p2.val)
  : (Subnet.mask p2) &&& (Subnet.mask p1) = Subnet.mask p1 := by
